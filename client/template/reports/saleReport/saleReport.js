...
Template.restaurant_saleReport.onRendered(function () {
    var name = $('[name="date"]');
    DateTimePicker.dateRange(name);
});
Template.restaurant_saleReport.events({
...
Template.restaurant_saleReportGen.helpers({
    ...
    data: function () {
        ...
        var call = Meteor.callAsync(callId, 'restaurantSaleReport', q);
        if (!call.ready()) {
            return false;
        }
        return call.result();
...
function calculateSaleHelper(sl) {
    ...
    sl.forEach(function (s) {
        ...
        s.order = i;
        s.exchangeRates = [];
        Restaurant.Collection.ExchangeRates.findOne(s.exchangeRateId).rates.
            forEach(function (ex) {
