            ...
            onOk: () => {
                Meteor.call('addSaleDetailToChef', data, (err, result) => {
                    if (err) {
                        ...
                        Bert.alert(`បញ្ជូនបានជោគជ័យ! ${data._product.name}`, 'success', 'growl-bottom-right', 'fa-check')
                    }
                });
            },
            onCancel: function () {
...
Template.saleDetail.helpers({
    checkMonitor(monitor, cookQty, quantity){
        if (!monitor || monitor && (cookQty < quantity)) {
            return true;
        }
        return false;
    },
    cookLeft(quantity, finishQty){
        return quantity - finishQty
    },
    checkIfIsCooking(){
        console.log(this.isCooking && this.quantity - this.cookQty != 0);
        if (!this.isCooking || this.isCooking && this.quantity - this.cookQty != 0) {
            return true;
        }
        return false;
    }
...
Template.saleInvoiceTotal.events({
    ...
    "click .add-all-sale-detail-to-monitor"(event, instance){
        var data = this;
        IonPopup.confirm({
            title: 'ចង់បញ្ជូនមុខទំនិញទាំងអស់ទៅចុងភៅ?',
            onOk: () => {
                Meteor.call('addAllSaleDetailToChef', data, (err, result) => {
                    if (err) {
                        Bert.alert(`បញ្ជូនមិនបានជោគជ័យ! ${data._product.name}`, 'danger', 'growl-bottom-right', 'fa-remove')
                    } else {
                        Bert.alert(`បញ្ជូនបានជោគជ័យ!`, 'success', 'growl-bottom-right', 'fa-check')
                    }
                });
            },
            onCancel: function () {
